...
enum AnyDieShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        switch self {
        case .roundedRectangle(let cr): return RoundedRectangle(cornerRadius: cr).path(in: rect)
        case .regularPolygon(let s):    return RegularPolygon(sides: s).path(in: rect)
...
func dieShape(sides: Int) -> AnyDieShape {
    switch sides {
